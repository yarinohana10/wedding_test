...
const DashboardGuests = () => {
  ...
  const handleUpdateField = async (guest: Guest, field: keyof Guest, value: any) => {
    try {
  ...
  const handleAddGuest = async () => {
    ...
    try {
      ...
      const { data: existingGuests, error: lookupError } = await supabase
        .from('guests')
        .select('phone')
        .eq('phone', newGuest.phone) as { data: { phone: string }[] | null; error: Error | null };
      ...
      if (existingGuests && existingGuests.length > 0) {
        toast({
          ...
          description: "מספר הטלפון שהזנת כבר קיים במערכת. כל מספר טלפון חייב להיות ייחודי.",
          variant: "destructive"
        });
        setIsLoading(false);
        return;
        ...
        try {
          ...
          for (let i = 0; i < jsonData.length; i++) {
            ...
            if (!/^\d{10}$/.test(phone.toString())) {
              errors.push(`שורה ${i + 1}: מספר טלפון ${phone} אינו תקין (נדרשים 10 ספרות)`);
              continue;
            }
            
            // Check if phone already exists in current db
            const existingInDB = guests.find(g => g.phone === phone.toString());
            if (existingInDB) {
              skipped++;
              continue;
            }
            ...
            const existingInNewGuests = newGuests.find(g => g.phone === phone.toString());
            if (existingInNewGuests) {
              errors.push(`שורה ${i + 1}: מספר טלפון ${phone} מופיע יותר מפעם אחת בקובץ`);
              continue;
            }
          ...
          toast({
            ...
            description: `יובאו ${newGuests.length} מוזמנים חדשים בהצלחה${skipped > 0 ? `, דילוג על ${skipped} מוזמנים קיימים` : ''}${errors.length > 0 ? `, ${errors.length} שגיאות` : ''}`,
          });
    ...
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        <div className="text-center">
          ...
          <Button className="mt-4 bg-wedding-primary hover:bg-wedding-accent text-white" onClick={fetchGuests}>נסה שנית</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <Card className="p-4 border-wedding-primary/20 bg-wedding-light">
          ...
          <div className="mt-2 flex items-center justify-between">
            <div className="h-12 w-12 rounded-full bg-wedding-primary/20 flex items-center justify-center">
              <span className="text-wedding-primary text-xl font-bold">{guests.length}</span>
            </div>
            <div className="text-right">
      ...
      {/* Desktop table view */}
      {!isMobile && (
        <div className="rounded-md border border-wedding-primary/20 bg-white">
          <Table>
            <TableHeader>
              <TableRow>
      ...
      <Dialog open={isImportDialogOpen} onOpenChange={setIsImportDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogHeader>
            <DialogTitle className="text-right">ייבוא מוזמנים מאקסל</DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <p className="text-sm text-gray-500 mb-6 text-right">
              העלה קובץ אקסל עם רשימת מוזמנים. הקובץ צריך להכיל עמודות "שם" ו"טלפון". 
              מספרי טלפון זהים לא יתווספו בשנית. ניתן להוריד תבנית לדוגמה.
            </p>
          ...
          <DialogFooter className="flex justify-between sm:justify-between">
            <Button variant="outline" onClick={() => setIsImportDialogOpen(false)}>ביטול</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
